...
export default async function handler(
  ...
  try {
    const response = await requestBackendAuth("/admin/login", {
      method: "POST",
    ...
    if (!response.ok || !payload || !("data" in payload)) {
      return res.status(response.status).json(
        payload ?? { message: "Falha ao autenticar admin" },
      );
    }
    ...
    return res.status(500).json({ message: `Erro no login: ${err.message}` });
  }
}